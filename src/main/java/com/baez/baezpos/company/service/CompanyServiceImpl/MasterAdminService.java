...
public class MasterAdminService implements MasterAdmin {
    ...
    public void registerFullBusiness(MasterRegistrationRequest req) {
        ...
        owner.setActive(true); // <--- AsignaciÃ³n garantizada en memoria y BD

        User savedOwner = userRepository.save(owner);

        // 3. EnvÃ­o directo de correo de bienvenida con sus credenciales de acceso
        try {
            ...
            log.error("ADVERTENCIA: La empresa ID {} se creÃ³ correctamente, pero fallÃ³ el envÃ­o del correo a {}: {}",
    ...
    public void updateCompanyMaster(Long id, CompanyDTO dto) {
        ...
        if (dto.getTaxId() != null && !dto.getTaxId().trim().equals(company.getTaxId())) {
            if (companyRepository.existsByTaxId(dto.getTaxId().trim())) {
                throw new IllegalArgumentException("El CUIT/TaxID ya estÃ¡ asignado a otra empresa.");
            }
    ...
    public void resetOwnerPassword(Long companyId, String newRawPassword) {
        User owner = userRepository.findByCompanyIdAndRole(companyId, Role.ADMIN).stream()
                .findFirst()
                .orElseThrow(() -> new ResourceNotFoundException("No se encontrÃ³ un administrador para esta empresa"));
        ...
        try {
            ...
            log.error("Error al enviar email de restablecimiento de contraseÃ±a a {}: {}", owner.getEmail(), e.getMessage());
        }
    }
