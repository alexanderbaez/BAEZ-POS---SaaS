...
public class SaleServiceImpl implements SaleService {
    ...
    public SaleResponseDTO createSale(SaleRequestDTO saleDTO, Long userId) {
        ...
        // MED-03: write-last â€” addBalance se ejecuta al final, garantizando que la venta,
        // el inventario y la cuenta corriente ya estÃ©n confirmados antes de actualizar el saldo de caja.
        if (!"CUENTA_CORRIENTE".equals(paymentMethodClean)) {
    ...
    private void handleCreditSale(Long customerId, Sale savedSale, Long companyId) {
        ...
        if (customer.getCreditLimit() != null && nuevoSaldo.compareTo(customer.getCreditLimit()) > 0) {
            throw new BadRequestException("La venta supera el lÃ­mite de crÃ©dito configurado para el cliente.");
        }
    ...
    public BoxReportDTO getBoxReport(String period, LocalDate from, LocalDate to) {
        ...
        // Total Ingresos Reales del DÃ­a (Ventas Efectivo/Transferencia + Cobros de Deudas)
        ...
        // 3. CAPA HISTÃ“RICA / RANGOS AUDITADOS (FLUJO DE CAJA PURO OPTIMIZADO VIA SQL)
        ...
        // Cobros de Cuenta Corriente en el perÃ­odo
        ...
        // FÃ“RMULA DE RECAUDACIÃ“N (FLUJO DE CAJA PURO):
        ...
        // netRevenue (Ganancia Neta) sobre ventas efectivamente cobradas menos costo de mercaderÃ­a
    ...
    public void cancelSale(Long saleId) {
        ...
        for (SaleItem item : sale.getItems()) {
            ...
            if (product != null) {
                inventoryService.registerMovement(
                        ...
                        item.getQuantity(),
                        MovementType.IN,
                        "DevoluciÃ³n por anulaciÃ³n de Ticket #" + sale.getNroComprobante()
                );
            }
        }

        // CompensaciÃ³n contable en cuenta corriente si el pago fue fiado / cuenta corriente
        if ("CUENTA_CORRIENTE".equalsIgnoreCase(sale.getPaymentMethod())) {
            ...
            if (movementOpt.isPresent()) {
                ...
                customerService.updateBalance(
                        ...
                        sale.getTotal(),
                        "CREDITO",
                        "CompensaciÃ³n por anulaciÃ³n de Ticket #" + sale.getNroComprobante(),
                        sale,
        ...
        try {
            ...
            log.error("Error al registrar auditorÃ­a de anulaciÃ³n para el Ticket #{}: {}", sale.getNroComprobante(), e.getMessage());
        }
    }
